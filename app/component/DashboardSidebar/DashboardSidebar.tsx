...
const DashboardSidebar = () => {
  ...
  return (
    <div
      className={`relative h-[79dvh] max-h-full bg-white shadow-sm w-[54]  overflow-clip  transition-all duration-200 ${
        sideOpen == true ? '!w-64' : 'hover:w-64'
      }`}
    >
        ...
        {hasRole('admin') && (
          <>
            <div className="mt-8 w-full">
              ...
              <div className="mt-2 space-y-1 pr-1 w-full">
                  ...
                  <Link
                    ...
                    className={`${
                      pathname === item.href
                        ? 'bg-blue-50 border-blue-500 text-blue-700'
                        : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                    } group flex items-center p-2 text-sm font-medium border-l-4 transition-colors duration-200 w-full text-nowrap`}
                  >
                    <span className="mr-3">{item.icon}</span>
                    {item.name}
                  </Link>
      ...
      <div className="absolute bottom-0 left-2 w-full border-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-900 ">
        <button
          className={`${sideOpen == true ? 'bg-gray-300' : ''} p-2 rounded-2xl`}
          onClick={handleSide}
        >
